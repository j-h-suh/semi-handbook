import { getExecChapterData, getSortedExecChapters } from '@/lib/markdown';
import MarkdownViewer from '@/components/MarkdownViewer';
import Link from 'next/link';
import { ArrowLeft, ArrowRight } from 'lucide-react';

export async function generateStaticParams() {
    const chapters = getSortedExecChapters();
    return chapters.map(c => ({ id: c.id }));
}

export default async function ExecChapterPage({ params }: { params: { id: string } }) {
    const { id: rawId } = await params;
    const id = decodeURIComponent(rawId);
    const chapterData = await getExecChapterData(id);
    const allChapters = getSortedExecChapters();
        ...
        <main className="flex-1 w-full flex overflow-hidden relative">
            <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 w-full">
                ...
                <div className="max-w-4xl mx-auto w-full px-8 py-12 lg:px-12 flex items-center justify-between border-t border-white/10 mt-8 mb-16">
                    {prevChapter ? (
                        <Link href={`/exec/${prevChapter.id}`} className="group flex flex-col items-start gap-2 p-4 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-all w-[48%] max-w-[320px]">
                            <span className="flex items-center gap-2 text-sm text-slate-400 group-hover:text-amber-400 transition-colors">
                    ...
                    {nextChapter ? (
                        <Link href={`/exec/${nextChapter.id}`} className="group flex flex-col items-end gap-2 p-4 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-all w-[48%] max-w-[320px] text-right">
                            <span className="flex items-center gap-2 text-sm text-slate-400 group-hover:text-amber-400 transition-colors">
