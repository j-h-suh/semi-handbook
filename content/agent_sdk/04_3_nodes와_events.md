...
> 💡 **4.2의 git을 이어서**: git이 떠오른다면 정확하다. `nodes`는 git의 *커밋 그래프*(누가 누구의 부모인가)에, `events`는 그 커밋이 담은 *변경 내용*에 대응한다. 다른 점은 우리가 그 그래프를 git의 객체 저장소가 아니라 평범한 관계형 테이블에 직접 그린다는 것뿐이다 — 우리 손으로, 우리 도메인의 연산 단위로.
...
> ⚠️ **코드 미검증 — 검증 레포 실행 필요**: 위 스키마·`Store`·재귀 CTE는 설계 초안이다. `_next_seq`의 동시성, `parent_id` 외래키 제약, `payload` JSON 직렬화 경계는 검증 레포에서 실제로 돌려 확인해야 한다. 본문은 *모델의 모양*을 보이는 데까지다.
