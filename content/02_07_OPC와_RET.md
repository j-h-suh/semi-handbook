...
소프트웨어 엔지니어에게 비유하면, OPE-는 **네트워크 전송에서의 신호 왜곡**과 같다. 송신(마스크)한 데이터와 수신(웨이퍼)한 데이터가 채널(광학 시스템)의 특성 때문에 달라진다. 이 "전송 왜곡"을 알고 있다면, 송신 측에서 미리 **반대로 왜곡**해두면 수신 측에서 원래 의도한 데이터를 복원할 수 있다. 이것이 바로 OPC의 핵심 아이디어다.
...
첫째, 계산 비용이 거대하다. 3 nm 칩 하나의 OPC에 수천 CPU 코어가 수일 동작한다. 이 비용을 10배 줄이면 칩 설계 비용이 의미 있게 감소한다.
