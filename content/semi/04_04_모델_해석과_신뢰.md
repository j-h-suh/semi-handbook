...
$$
\hat{y} = \underbrace{\bar{y}}_{\text{base}} + \sum_{i=1}^{N} \phi_i \quad \text{(SHAP value of feature } i\text{)}
$$

### 구체적 예시: CD 예측


![SHAP Waterfall Plot — CD 예측 해석](/content/semi/images/04_04/shap_waterfall_cd.png)

$$
\begin{aligned}
&\text{base CD (mean):}\; 20.0\text{nm} \\
&+ \phi_{\text{PEB\_temp}} = +0.8\text{nm} \quad \leftarrow \text{largest positive} \\
&+ \phi_{\text{Dose}} = +0.5\text{nm} \\
&+ \phi_{\text{Resist}} = +0.3\text{nm} \\
&+ \phi_{\text{Equip}} = -0.2\text{nm} \\
&+ \phi_{\text{Focus}} = -0.1\text{nm} \\
&= \hat{y} = 21.3\text{nm}
\end{aligned}
$$

이제 엔지니어에게 "이 웨이퍼의 CD가 1.3 nm 높은 이유는 **PEB 온도가 높고(+0.8 nm) Dose가 낮았기(+0.5 nm) 때문**"이라고 구체적으로 설명할 수 있다. 엔지니어는 이 설명이 자신의 경험과 일치하는지 확인할 수 있고, 일치하면 모델을 **신뢰**하게 된다.

### SHAP의 네 가지 시각화


![SHAP Beeswarm Plot — 전체 피처 기여 분포](/content/semi/images/04_04/shap_beeswarm_cd.svg)
...
**크기(Magnitude)** — Dose 1% 변화에 대한 CD 변화량이 경험적으로 알려진 값(~0.5~1 nm)과 크기가 일치하는지 확인한다. 10 nm 변한다면 모델이 잘못된 것이다.
...
![Physics-Informed 하이브리드 모델 구조](/content/semi/images/04_04/physics_informed_hybrid_model.svg)
...
$$
\mathcal{L} = \text{MSE}(y, \hat{y}) + \lambda \cdot \text{Physics\_Violation\_Penalty}
$$
...
$$
\hat{y} = \underbrace{f_{\text{physics}}(X)}_{\text{Correctables}} + \underbrace{g_{\text{ML}}(X)}_{\text{Residuals}}
$$
...
![신뢰 구축 로드맵 — Shadow에서 Full Auto까지](/content/semi/images/04_04/trust_building_roadmap.svg)
