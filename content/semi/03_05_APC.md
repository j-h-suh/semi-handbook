...
![공정 제어 체계 — SPC · FDC · APC](/content/semi/images/03_05/apc_control_hierarchy.svg)
...
간단한 예로 이해해 보자. CD 목표가 20 nm인데, 로트 N의 측정 결과가 21 nm(+1 nm 초과)다. APC는 다음 로트 N+1에서 Dose를 약간 올려(포지티브 레지스트에서 Dose↑ → CD↓ 관계를 이용) CD를 20 nm에 가깝게 맞춘다. 로트 N+1의 측정 결과가 19.8 nm(-0.2 nm)이면, 다음 로트 N+2에서 Dose를 살짝 줄인다. 이렇게 매 Run의 결과를 보고 다음 Run을 조정하는 반복이 R2R 제어다.
...
$$
\hat{y}_{n+1} = \lambda \cdot y_n + (1 - \lambda) \cdot \hat{y}_n
$$
...
![EWMA R2R 제어 루프](/content/semi/images/03_05/ewma_control_loop.svg)
...
![Feed-Forward + Feedback 결합 보정 개념도](/content/semi/images/03_05/ff_fb_combined_correction.svg)

### Feedback: 지나간 것에서 배운다

**Feedback(피드백) 보정**은 이전 Run의 계측 결과를 바탕으로 다음 Run의 공정 조건을 조정한다. "로트 N의 CD가 목표보다 1 nm 컸으니, 로트 N+1에서 Dose를 올리자"는 식이다.
...
$$
\text{보정값} = \underbrace{\text{Feed-Forward}}_{\text{현재 웨이퍼}} + \underbrace{\text{Feedback}}_{\text{이전 로트 추세}} + \underbrace{\text{고정 오프셋}}_{\text{장비 고유}}
$$
...
![APC 보정 전/후 CD 분포 비교](/content/semi/images/03_05/apc_cd_before_after.png)

CD APC의 1차 도구는 **Dose 조정**이다. 2.11장에서 Dose가 CD에 가장 직접적인 영향을 미친다고 배웠다(~1 nm per 1% Dose).

$$
\Delta\text{Dose} = f(\text{CD\_error}_{\text{prev}},\; \text{Resist\_thickness},\; \Delta T_{\text{PEB}},\; \ldots)
$$
...
$$
\text{Correction Efficiency} = \frac{\sigma_{\text{Raw}} - \sigma_{\text{Residual}}}{\sigma_{\text{Raw}}} \times 100\%
$$
...
![Cross-Layer APC 개념 — 다층 통합 최적화](/content/semi/images/03_05/cross_layer_apc.svg)
