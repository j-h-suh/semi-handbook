...
> 💡 **head 하나 = 시각 하나**: 학습이 끝난 Transformer를 열어 보면, 어떤 head는 바로 앞 단어에, 어떤 head는 문장의 동사에, 어떤 head는 짝이 되는 괄호에 집중하는 모습이 *관찰되곤 한다*. 우리가 "너는 문법을 봐"라고 지정하지 않아도 역할이 갈린다. 다만 모든 head가 이렇게 깔끔히 해석되는 건 아니다 — 역할이 모호하거나 서로 겹쳐 보이는 head도 많다(7.3에서 직접 들여다본다).

그런데 왜 head들이 *똑같아지지 않고* 갈라질까. 같은 구조를 여러 개 두면 다 같은 걸 배울 것 같은데 말이다. 두 가지가 분화를 만든다. 첫째, 각 head의 투영 행렬을 *서로 다른 난수*로 초기화해 출발선을 어긋나게 둔다 — 1권에서 본 대칭 깨기다. 둘째, 학습이 그 차이를 키운다. head 둘이 똑같은 일을 하면 한쪽은 군더더기여서 손실을 더 줄이지 못하니, 학습은 자연히 각 head를 *다른* 쓸모로 밀어 보낸다. 중복보다 분업이 손실에 유리하기 때문이다.
...
> ✅ **코드 검증됨 — `playground/handbook/llm` 프로브 통과.** 이 장의 `MultiHeadAttention`은 검증 레포에서 실행 확인했다(probe_1_forward). `view`/`transpose`/`contiguous`의 shape 흐름과 head 분할·복원 정상.
