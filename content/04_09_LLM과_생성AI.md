...
이전 챕터들에서 다룬 VM, APC, FDC, 웨이퍼맵 분류 등은 모두 **정량적 예측/분류** 문제였다 — 센서 데이터를 입력받아 CD를 예측하고, 이미지를 입력받아 결함 유형을 분류하는. 이것은 LLM의 역할이 **아니다**. LLM은 "CD = 21.3 nm"를 예측하는 데 쓰이지 않는다.
...
**정량적 정확도 부족** — "CD = 21.3 nm"같은 정확한 수치 예측은 VM의 역할이지 LLM의 역할이 아니다. LLM은 "CD가 높아질 가능성이 있습니다" 수준의 정성적 판단은 가능하지만, 0.1 nm 단위의 정량적 예측은 불가능하다.
