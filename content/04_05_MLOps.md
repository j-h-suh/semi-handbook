...
1. **새 데이터 축적** → 재학습 트리거 감지
2. **새 모델 학습** (Sliding Window 기반)
3. **오프라인 검증** — 성능(RMSE, R²) + 물리 정합성(Dose↑→CD↓)
4. **Shadow 테스트** — 병렬 실행, 실 적용 없이 기존 모델과 성능 비교
5. 성능 확인 **OK** → 모델 교체 (Production 승격)
6. 성능 확인 **NG** → 롤백, 원인 분석
