...
## Phase 26: claude 다이어그램 batch 2 — 03_2 + 10_1 *(✅ 완료)*

2026-05-23 사용자 _"갑시다"_ — 5.1 / 6.3 / 8.4 시도, _작은 TUI / 코드 자리만_ 발견 → 다른 챕터 (03_2, 10_1) 의 _큰 자리_ 진행.

### 26-1. 변환 (commit `c9ffc96`)

| 자리 | 컴포넌트 | 내용 |
|---|---|---|
| 03_2 (38 줄) | `ToolInterface47Fields.tsx` | 5 카테고리 카드 (정체성 9 / 스키마 3 / 실행 9 / 권한 11 / 렌더링 15 = 47) + 필드 칩 |
| 10_1 (38 줄) | `Part10Architecture.tsx` | 9.1 위에 10.1~10.8 _확장 자리_ 표시 (`┊ 10.x`), main → query (내부 ①~④ + while + yield chunk) → message_queue 공유 채널 |

claudeRegistry 6 자리 누적.

### 26-2. 진단 (변환 _안 함_)

- **5.1 BLOCK 1** (Settings UI 6 줄) — _TUI 메뉴 시뮬_, ASCII 가 _자연_ — 보존
- **5.1 BLOCK 2/3, 5.2 BLOCK 1** — _TUI 화면 시뮬_ (입력/스피너/푸터) — ASCII 가 _실제 터미널 모양_ 자체 — 보존
- **6.3 BLOCK 2, 8.4 BLOCK 1/2** — `\`\`\`typescript`/`\`\`\`python` 코드 (awk 가 lang 자리 _스킵 못 함_ — _``` 매칭 false positive_), ASCII 다이어그램 _없음_

### 26-3. 남은 우선순위

| 다음 | 챕터 | 자리 |
|---|---|---|
| 1 | 09_1 디렉토리 트리 | 보존 결정 (트리 패턴이 코드 폰트와 자연) |
| 2 | 06_6 Hook 실전 (46 chars) | _실행 흐름_ — ASCII 자리 _발굴 필요_ |
| 3 | 05_2 selector 분기 (line 204~220) | _다중 자리_ |
| 4 | 10_x 후속 (10_2~10_8) | Part 10 확장 자리들 |
| 5 | _작은 자리 누적_ — 전체 chap 별 ASCII 자리 정확 분석 후 _점진_ |
...
## Phase 28: claude 다이어그램 batch 4 — 10_3 + 10_5 + 08_4 *(✅ 완료)*

2026-05-23 사용자 _"계속 이어서 합시다"_ — box-drawing _첫 줄_ (┌) 기준 발굴 (awk false positive 회피).

### 28-1. 변환 (commit `bd72a5b`)

| 자리 | 컴포넌트 | 내용 |
|---|---|---|
| 10_3 line 117~143 (27 줄) | `HookLifecycle.tsx` | Hook 3 단계 수직 카드 (Event → 매칭 → 실행 → 반영) |
| 10_5 line 235~251 (17 줄) | `APIClientAdapter.tsx` | agent.py 분기 — AsyncAnthropic / VLLMClient (어댑터) → openai → vLLM |
| 08_4 line 394~419 (26 줄) | `QueueLoopModel.tsx` | query loop 5 단계 + queue 별도 박스 (사용자/워커/팀원/시스템) |

### 28-2. 누적 다이어그램 11 자리

`09_1` / `02_1` / `00_1 (×2)` / `03_1` / `03_2` / `10_1` / `05_2` / `10_3` / `10_5` / `08_4`

### 28-3. 남은 자리 (chap 별 발굴)

| 챕터 | 박스 수 | 상태 |
|---|---|---|
| 10_3 | 4 | 1 변환 완료, 나머지 3 _점진_ |
| 06_1~06_4 | 1 each | _작은 자리_, 점진 |
| 10_2, 10_4, 10_6, 10_7, 10_8 | 0~? | 발굴 필요 |
| 07_x | 0~? | 발굴 필요 |

### 28-4. 다음 batch 후보

- 10_3 의 _나머지 3 박스_ (실전 hook 예시 다이어그램)
- 06_x 시리즈 — 1 박스씩, 권한/Hook 자리
- 10_2/10_4/10_6/10_7/10_8 — 추가 발굴
