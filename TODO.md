...
## Phase 5: claude_code 본문 수정·보완 *(1순위 — 다음 작업)*
...
## Phase 7: Deep Dive 콜아웃 포맷 통일 *(2순위 — 출간 직전 정리)*
...
- [ ] 01_2_REPL_루프 (1건)
- [ ] 02_1_에이전트_루프_한_장 (1건)
- [ ] 02_2_비동기_제너레이터_파이프라인 (3건)
- [ ] 02_3_스트리밍과_재시도 (1건)
- [ ] 03_1_도구란_무엇인가 (2건)
- [ ] 03_2_Tool_인터페이스_47개_필드 (1건)
- [ ] 03_3_buildTool_팩토리 (1건)
- [ ] 03_4_단순한_도구_FileRead (1건)
- [ ] 03_5_복잡한_도구_BashTool (2건)
- [ ] 04_1_슬래시_명령은_일반_채팅과_어떻게_다른가 (1건)
- [ ] 04_2_세_가지_명령_타입 (1건)
- [ ] 04_3_allowedTools (1건)
- [ ] 05_1_터미널에_React (1건)
- [ ] 05_2_AppState_스토어 (1건)
- [ ] 05_3_메시지_렌더링과_가상_스크롤 (1건)
- [ ] 05_4_스트리밍_텍스트와_부분_업데이트 (1건)
- [ ] 06_1_CLAUDE_md (1건)
- [ ] 06_2_AppState (1건)
- [ ] 06_3_권한_모드_5가지 (1건)
- [ ] 06_4_권한_규칙_매칭 (1건)
...
- [ ] 07_1_API_클라이언트 (1건)
- [ ] 10_1_사용자_정의_슬래시_명령 (1건)
- [ ] 10_2_스킬_만들기 (1건)
- [ ] 10_3_MCP_서버_만들기 (1건)

### 추가 검토 주제 (챕터별 리스트업 보류, 큰 주제로만 보존)

- [ ] **콜아웃(💡 등) 포맷도 동일 패턴으로 통일** — 짧은 보충은 펼친 채로, 긴 보충은 `<details>` + blockquote로 접기.
- [ ] **콜아웃과 Deep Dive의 시각적 위계 구분** — 둘 다 박스로 보이면 경계가 흐려진다. 아이콘(💡 vs 🔬), 접기 여부, 색/들여쓰기 등으로 위계 분리.
- [ ] **Python 등가 코드 누락 점검** — TS 코드 블록 뒤에 Python 등가가 `:::tabs` 짝으로 따라붙는 패턴이 전 챕터에 일관되게 적용됐는지, 누락된 TS-only 블록이 있는지 확인.
